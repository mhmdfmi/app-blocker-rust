...
impl AppEngine {
    ...
    pub fn run(mut self, shutdown_flag: Arc<std::sync::atomic::AtomicBool>) {
                ...
                Ok(event) => {
                    ...
                    if let Err(e) = self.handle_event(event) {
                        ...
                        if errors >= 5 {
                            ...
                            let _ = self
                                .state_manager
                                .force_safe_mode("consecutive_errors_threshold");
                        }
                    }
                }
        ...
        // Audit shutdown
        audit(AuditEntry::new(AuditEventKind::SystemStopped).with_detail("Shutdown normal"));
    ...
    fn handle_event(&mut self, event: AppEvent) -> AppResult<()> {
        ...
        match event {
            ...
            AppEvent::UnlockFailed {
                ...
                audit(
                    AuditEntry::new(AuditEventKind::AuthFailed)
                        .with_trace(&trace_id)
            ...
            AppEvent::EmergencyUnlock { trace_id } => {
                ...
                audit(AuditEntry::new(AuditEventKind::EmergencyUnlock).with_trace(&trace_id));
            ...
            AppEvent::EnterSafeMode { reason } => {
                audit(AuditEntry::new(AuditEventKind::SafeModeEntered).with_detail(&reason));
    ...
    fn handle_process_detected(
        ...
        audit(
            AuditEntry::new(AuditEventKind::ProcessDetected)
                .with_trace(&trace_id)
        ...
        self.state_manager
            .transition_to(AppState::Blocking, "process_detected")?;
        ...
        match kill_result {
            Ok(terminated) => {
                ...
                self.state_manager
                    .transition_to(AppState::Locked, "process_blocked")?;
                self.state_manager.update_data(|d| {
                    d.overlay_active = true;
                })?;

                audit(
                    AuditEntry::new(AuditEventKind::OverlayShown)
                        .with_trace(&trace_id)
                        .with_process(info.pid, &info.name),
                );
            ...
            Err(e) => {
                audit(
                    AuditEntry::new(AuditEventKind::ProcessKillFailed)
                        .with_trace(&trace_id)
                        .with_process(info.pid, &info.name)
                ...
                self.state_manager
                    .transition_to(AppState::Recovering, "kill_failed")?;
                self.state_manager.reset_data()?;
                self.state_manager
                    .transition_to(AppState::Monitoring, "recovery_after_kill_fail")?;
            }
        }
    ...
    fn handle_unlock_success(&mut self, trace_id: uuid::Uuid, username: &str) -> AppResult<()> {
        ...
        audit(
            AuditEntry::new(AuditEventKind::AuthSuccess)
                .with_trace(&trace_id)
        ...
        audit(
            AuditEntry::new(AuditEventKind::OverlayDismissed)
                .with_trace(&trace_id)
                .with_duration(session_duration),
        );
        ...
        self.state_manager
            .transition_to(AppState::Monitoring, "recovery_complete")?;
