...
use crate::core::state::{AppState, SharedState, State};
...
impl Engine {
    ...
    fn handle_event(&mut self, event: AppEvent) -> AppResult<()> {
        ...
        tracing::debug!(
            "Handling event: {:?} (trace: {})",
            std::mem::discriminant(&event),
            trace_id
        );

        match event {
            AppEvent::ProcessDetected(info) => self.handle_process_detected(info, &trace_id),
            AppEvent::ProcessBlocked(info) => self.handle_process_blocked(info, &trace_id),
            ...
            AppEvent::ShutdownRequested => self.handle_shutdown(),
            AppEvent::EmergencyUnlock => self.handle_emergency_unlock(),
            AppEvent::Error(err) => self.handle_error(err),
            _ => Ok(()),
        }
    }
    ...
    fn handle_overlay_requested(
        &mut self,
        request: crate::core::events::OverlayRequest,
        trace_id: &str,
    ) -> AppResult<()> {
        tracing::info!(
            "Overlay requested for process: {} (trace: {})",
            request.process_info.name,
            trace_id
        );
